# 🚀 WSL環境でのテスト実行 - クイックリファレンス

## ❌ 発生したエラー

```
'\\wsl.localhost\Ubuntu-20.04\home\user1\DockerDashBord\graphql-server'
上記の現在のディレクトリで CMD.EXE を開始しました。
UNC パスはサポートされません。Windows ディレクトリを既定で使用します。
'NODE_OPTIONS' は、内部コマンドまたは外部コマンド、
操作可能なプログラムまたはバッチ ファイルとして認識されていません。
```

## ✅ 原因

Windows側（エクスプローラー、コマンドプロンプト、PowerShell）から
WSLのファイルにアクセスする際の**UNCパス**（`\\wsl.localhost\...`）では、
Node.js環境変数やnpmスクリプトが正常に動作しません。

## 🎯 解決策（3つの方法）

---

### 方法1: WSLターミナル内で実行【推奨】⭐

最もシンプルで確実な方法です。

#### ステップ

```bash
# 1. WSLターミナルを開く
wsl

# 2. プロジェクトディレクトリへ移動
cd ~/DockerDashBord

# 3. パスを確認（UNCパスでないことを確認）
pwd
# 正常: /home/user1/DockerDashBord
# 異常: /mnt/c/... または \\wsl.localhost\...

# 4. テストを実行
./run-all-tests.sh
```

#### WSLターミナルの開き方

**方法A: コマンドで開く**
```bash
# PowerShellまたはコマンドプロンプトで
wsl
```

**方法B: スタートメニューから**
- Windows スタートメニュー → 「Ubuntu」または「WSL」を検索
- クリックして起動

**方法C: Windows ターミナルから**
- Windows ターミナルを開く
- タブの「+」→ Ubuntu（またはインストール済みのLinuxディストリビューション）

**方法D: VSCode から**
- プロジェクトフォルダを開く
- `Ctrl + Shift + P` → "WSL: Reopen Folder in WSL"
- 統合ターミナルを開く（`Ctrl + `` ）

---

### 方法2: Windows用ラッパースクリプト【便利】🔧

Windows側から自動的にWSL内でテストを実行します。

#### 使い方

```powershell
# PowerShellで実行
.\run-tests-wsl.ps1
```

#### 機能
- ✅ WSLの利用可能性を自動確認
- ✅ プロジェクトパスを自動検出
- ✅ WSL内で正しくテストを実行
- ✅ エラー時にトラブルシューティング情報を表示

#### 実行前の準備

```powershell
# 最新版を取得
git pull origin main

# スクリプトの実行権限を確認（PowerShellで必要な場合）
Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned
```

---

### 方法3: wslコマンドで直接実行【上級者向け】💻

PowerShellからwslコマンドを使用します。

```powershell
# プロジェクトパスを指定して実行
wsl bash -c "cd ~/DockerDashBord && ./run-all-tests.sh"

# または npm スクリプトで
wsl bash -c "cd ~/DockerDashBord && npm run test:all"
```

---

## 📋 完全な実行手順（初回）

WSL環境で初めてテストを実行する場合の完全な手順：

```bash
# 1. WSLに入る
wsl

# 2. プロジェクトディレクトリへ移動
cd ~/DockerDashBord

# 3. 最新版を取得
git pull origin main

# 4. スクリプトに実行権限を付与
chmod +x run-all-tests.sh

# 5. 依存関係をインストール
npm run install:all

# 6. Dockerを起動（E2Eテスト用）
docker compose up -d

# 7. Docker起動確認
docker compose ps
# すべてのサービスが "Up" であることを確認

# 8. テストを実行
./run-all-tests.sh
```

---

## 🛠️ トラブルシューティング

### 問題: WSLに入れない

```powershell
# PowerShell（管理者権限）で
wsl --shutdown
wsl
```

### 問題: プロジェクトが見つからない

```bash
# WSL内で
cd ~
find . -name "DockerDashBord" -type d 2>/dev/null
```

### 問題: 権限エラー

```bash
chmod +x run-all-tests.sh
```

### 問題: Dockerが起動しない

```bash
# Docker Desktop（Windows側）が起動しているか確認
# WSL統合が有効か確認: Docker Desktop > Settings > Resources > WSL Integration
```

---

## 📊 実行結果の見方

成功した場合:
```
==========================================
🧪 Hospital Dashboard - All Tests Runner
==========================================

✅ Server Unit Tests PASSED
✅ Server Integration Tests PASSED
✅ Client Component Tests PASSED
✅ E2E Tests PASSED

📊 Test Results Summary
✅ Passed: 4
❌ Failed: 0

🎉 All tests passed!
```

失敗した場合:
```
❌ Server Unit Tests FAILED
...
💥 Some tests failed!
```

個別のログを確認:
```bash
# サーバーテストの詳細
cd graphql-server && npm test -- --verbose

# クライアントテストの詳細
cd dashboard && npm test

# E2Eテストの詳細（ブラウザ表示）
cd e2e-tests && npm run test:headed
```

---

## 🔗 関連ドキュメント

- **[WSL_EXECUTION_GUIDE.md](./WSL_EXECUTION_GUIDE.md)** - WSL実行の詳細ガイド
- **[TROUBLESHOOTING.md](./TROUBLESHOOTING.md)** - 全般的なトラブルシューティング
- **[TEST_QUICKSTART.md](./TEST_QUICKSTART.md)** - テスト実行のクイックガイド
- **[TESTING.md](./TESTING.md)** - テスト戦略とTDD詳細

---

## 💡 ベストプラクティス

### 開発時

```bash
# WSLターミナルで開発
wsl
cd ~/DockerDashBord

# ウォッチモードで開発
cd graphql-server && npm run test:watch
# または
cd dashboard && npm run test:watch
```

### コミット前

```bash
# すべてのテストを実行
./run-all-tests.sh

# すべてパスしたらコミット
git add .
git commit -m "..."
git push
```

---

## まとめ

**最も簡単な方法:**

1. `wsl` コマンドを実行
2. `cd ~/DockerDashBord`
3. `./run-all-tests.sh`

**これでUNCパスエラーは完全に解決します！** ✅

---

## 🆘 さらに困ったら

1. **ドキュメントを確認**
   - [WSL_EXECUTION_GUIDE.md](./WSL_EXECUTION_GUIDE.md)
   - [TROUBLESHOOTING.md](./TROUBLESHOOTING.md)

2. **ログを確認**
   ```bash
   docker compose logs -f
   ```

3. **完全リセット**
   ```bash
   docker compose down -v
   docker compose up -d --build
   npm run install:all
   ```

4. **GitHub Issues**
   - https://github.com/hiro1966/DockerDashBord/issues
